...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, sid, tid, lid, spk_emb) in enumerate(loader):
        ...
        spk_emb = spk_emb.cuda(rank, non_blocking=True)
        ...
        with autocast("cuda", enabled=hps.train.fp16_run):
            y_hat, y_hat_mb, l_length, attn, ids_slice, x_mask, z_mask, (z, z_p, m_p, logs_p, m_q, logs_q), (
                hidden_x, logw, logw_) = net_g(x, x_lengths, spec, spec_lengths, sid=sid, tid=tid, lid=lid, spk_emb=spk_emb)
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, sid, tid, lid, spk_emb) in enumerate(eval_loader):
            x, x_lengths = x.cuda(0), x_lengths.cuda(0)
            spec, spec_lengths = spec.cuda(0), spec_lengths.cuda(0)
            y, y_lengths = y.cuda(0), y_lengths.cuda(0)
            sid, tid, lid = sid.cuda(0), tid.cuda(0), lid.cuda(0)
            spk_emb = spk_emb.cuda(0)
            ...
            y_hat, y_hat_mb, l_length, attn, ids_slice, x_mask, z_mask, (z, z_p, m_p, logs_p, m_q, logs_q), _ = generator(
                x, x_lengths, spec, spec_lengths, sid=sid, tid=tid, lid=lid, spk_emb=spk_emb
            )
